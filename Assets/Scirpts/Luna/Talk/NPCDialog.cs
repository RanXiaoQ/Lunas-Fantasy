...
public class NPCDialog : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameManager.m_Instance.m_DialogInfoIndex = 0;
        contentIndex = 1;
    }
    /// <summary>
    /// 显示对话内容
    ...
    {
        if (GameManager.m_Instance.m_DialogInfoIndex > 7)
        {
            return;
        }
        if (contentIndex >= dialogInfoList[GameManager.m_Instance.m_DialogInfoIndex].Length)
        {
            if (GameManager.m_Instance.m_DialogInfoIndex == 2 &&
                !GameManager.m_Instance.m_HasPetTheDog)
            {

            }
            else if (GameManager.m_Instance.m_DialogInfoIndex == 4 &&
                GameManager.m_Instance.m_CandleNum < 5)
            {

            }
            else if (GameManager.m_Instance.m_DialogInfoIndex == 6 &&
                GameManager.m_Instance.m_KillNum < 5)
            {

            }
            else
            {
                GameManager.m_Instance.m_DialogInfoIndex++;
            }
            if (GameManager.m_Instance.m_DialogInfoIndex == 6)
            {
                GameManager.m_Instance.ShowMonsters();
            }

            //当前这段对话结束了，可以开始控制luna
            contentIndex = 0;
            UIManager.m_Instance.ShowDialog();
            GameManager.m_Instance.m_CanControlLuna = true;
        }
        else
        {
            DialogInfo dialogInfo = dialogInfoList[GameManager.m_Instance.m_DialogInfoIndex][contentIndex];
            UIManager.m_Instance.ShowDialog(dialogInfo.content, dialogInfo.name);
            contentIndex++;
    ...
    {
        contentIndex = dialogInfoList[GameManager.m_Instance.m_DialogInfoIndex].Length;
    }
}
/// <summary>
/// 类外的结构体 对话信息
